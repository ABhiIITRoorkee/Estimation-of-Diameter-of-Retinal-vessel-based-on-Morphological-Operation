...
# Instantiate the Data class with parameters from args
data_loader = Data(args.data_path + args.dataset, batch_size=args.batch_size)
def create_adj_cat(node_features, n_components):
    from sklearn.mixture import GaussianMixture
    gmm = GaussianMixture(n_components=n_components, random_state=42)
    labels = gmm.fit_predict(node_features)
    n_nodes = node_features.shape[0]
    adj_cat = np.zeros((n_nodes, n_nodes))
    for i in range(n_nodes):
        for j in range(n_nodes):
            adj_cat[i, j] = 1 if labels[i] == labels[j] else 0
    adj_cat = torch.FloatTensor(adj_cat)
    return adj_cat

node_features_df = data_loader.load_node_features()
node_features = node_features_df[['feature1', 'feature2']].values
adj_cat = create_adj_cat(node_features, n_components=5)
print("sahpe of adj_cat",adj_cat.shape )
...
class Model_Wrapper(object):
    def __init__(self, data_config, pretrain_data,data_loader):
        # # argument settings
        ...
        # print('----self.alg_type is {}----'.format(self.alg_type))
        # self.norm_u1, self.norm_u2, self.norm_i1, self.norm_i2,self.adj_cat = self.build_hyper_edge(
        ...
        # self.adj_cat = adj_cat.cuda()  # Move adj_cat to GPU

        # if self.alg_type in ['hcf']:
        #     self.model = HCF(self.n_users, self.n_items, self.emb_dim, self.layer_num, self.mess_dropout,self.adj_cat)
        # else:
    ...
    def build_hyper_edge(self, file):
        ...
        user_inter = np.zeros((self.data_loader.n_users, self.data_loader.n_items))
        items_inter = np.zeros((self.data_loader.n_items, self.data_loader.n_users))

        node_features = self.data_loader.load_node_features()
        with open(file) as f:
        ...
        # Load or generate node features and create adj_cat using Gaussian Mixture
        
        # Calculate normalized matrices
        # self.norm_u1 = self.sparse_mx_to_torch_sparse_tensor(sp.csr_matrix(np.dot(self.HuEdge, self.HuEdge.T)))
        # self.norm_u2 = self.sparse_mx_to_torch_sparse_tensor(sp.csr_matrix(np.dot(self.HuEdge_T, self.HuEdge_T.T)))
        # self.norm_i1 = self.sparse_mx_to_torch_sparse_tensor(sp.csr_matrix(np.dot(self.HiEdge, self.HiEdge.T)))
        # self.norm_i2 = self.sparse_mx_to_torch_sparse_tensor(sp.csr_matrix(np.dot(self.HiEdge_T, self.HiEdge_T.T)))

        return self.norm_u1, self.norm_u2, self.norm_i1, self.norm_i2,self.adj_cat
    ...
    def test(self, users_to_test, drop_flag=False, batch_test_flag=False):
        ...
        with torch.no_grad():
            ua_embeddings, ia_embeddings = self.model(self.norm_u1, self.norm_u2, self.norm_i1, self.norm_i2,adj_cat)
...
if __name__ == '__main__':
    ...
    args = parse_args()  # Ensure args are parsed or set up correctly
    data_loader = Data(args.data_path + args.dataset, batch_size=args.batch_size)  # Ensure this is correct
    config = {
    ...
    Engine = Model_Wrapper(data_config=config, pretrain_data=pretrain_data,data_loader=data_loader)
    if args.pretrain:
